...
const MProducto = () => {
  ...
  useEffect(() => {
    const storedData = JSON.parse(localStorage.getItem("appData")) || { productos: [], categorias: [] };
    
    // Solo establecer categorías predeterminadas si no hay categorías en localStorage
    ...
    const categoriesToUse = storedData.categorias.length > 0 ? storedData.categorias : defaultCategories;
    
    setProducts(storedData.productos || []);
    setCategories(categoriesToUse);

    // Guardar datos iniciales solo si se usaron categorías predeterminadas
    if (storedData.categorias.length === 0) {
      localStorage.setItem("appData", JSON.stringify({
        productos: storedData.productos || [],
        categorias: categoriesToUse,
      }));
    }
  }, []);
